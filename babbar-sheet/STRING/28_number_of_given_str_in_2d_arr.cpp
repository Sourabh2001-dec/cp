  ...
  for (r = 0; r < row_count; ++r)
  {
    for (c = 0; c < col_count; ++c)
    {
      if (str[r][c] == needle[0])
      {
        ...
        // right
        while (j < col_count && needlePtr < needle.length())
        {
          if (str[i][j] == needle[needlePtr])
          {
            matchedLen++;
          }
          else
        ...
        needlePtr = 0;
        matchedLen = 0;
        while (i >= 0 && needlePtr < needle.length())
        {
          if (str[i][j] == needle[needlePtr])
          {
            matchedLen++;
        ...
        needlePtr = 0;
        matchedLen = 0;
        while (i < row_count && needlePtr < needle.length())
        {
          if (str[i][j] == needle[needlePtr])
          {
            matchedLen++;
        ...
        needlePtr = 0;
        matchedLen = 0;
        while (j >= 0 && needlePtr < needle.length())
        {
          if (str[i][j] == needle[needlePtr])
          {
            matchedLen++;
